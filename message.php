...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        function showMessage(nameStr, contentStr, color) {
                ...
                var curIndex = 0;
                while(curIndex<contentStr.length){
                    var urlIndex = contentStr.indexOf("http", curIndex);
                    if(urlIndex >= 0){
                        var notUrlContent = contentStr.substring(0,urlIndex);
                        contentNode.appendChild(document.createTextNode(notUrlContent));
                        curIndex = contentStr.indexOf(" ",urlIndex);
                        if(curIndex == -1 ){
                            curIndex = contentStr.length;
                        }
                        var urlLink = contentStr.substring(urlIndex,curIndex);
                        var linkNode = document.createElementNS("http://www.w3.org/2000/svg", "a");
                        linkNode.setAttributeNS('http://www.w3.org/1999/xlink', 'xlink:href', urlLink);
                        linkNode.setAttributeNS('http://www.w3.org/1999/xlink', 'xlink:show', "new");
                        linkNode.setAttribute("style", "text-decoration: underline");

                        linkNode.appendChild(document.createTextNode(urlLink));
                        contentNode.appendChild(linkNode);
                        contentStr = contentStr.substring(curIndex);
                        curIndex = 0;
                    }
                    else{
                        contentNode.appendChild(document.createTextNode(contentStr));
